...
export default function UploadReceipt({ projectId, hasChargeSlip }: UploadReceiptProps) {
  ...
  const handleUpload = async () => {
    ...
    setUploading(true);
    try {
      // Upload file to Storage: receipts/{projectId}/{uuid}.ext
      const downloadURL = await uploadFile(pendingFile, `receipts/${projectId}`);

      // Save receipt metadata in Firestore subcollection
      ...
      // Auto-fill the linked charge slip's orNumber and dateOfOR if not already set
      try {
        const chargeSlips = await getChargeSlipsByProjectId(projectId);
        for (const cs of chargeSlips) {
          if (cs.id && cs.status !== "paid" && cs.status !== "cancelled") {
            await updateChargeSlip(cs.id, {
              orNumber: cs.orNumber || orNumber.trim(),
              dateOfOR: cs.dateOfOR || Timestamp.fromDate(new Date(orDate)),
            });
          }
        }
      } catch {
        // Non-critical: charge slip update failed silently
      }
  ...
  const handleDelete = async (receipt: Receipt) => {
    ...
    try {
      ...
      await logActivity({
        ...
        entityType: "project",
        entityId: projectId,
        description: `Deleted returned official receipt: ${receipt.fileName || receipt.id}`,
      });
      toast.success("Receipt removed. You may now upload a corrected version.");
    } catch (err) {
    ...
    <div className="space-y-2 mt-1">
      ...
      {loadingReceipts ? (
        ...
        <div className="space-y-1.5 ml-5">
          ...
          {/* Returned receipt prompt */}
          {receipts.some((r) => r.returnedByAdmin && !r.acknowledgedByAdmin) && (
            <p className="text-[10px] text-rose-600 ml-1 mt-1">
              ⚠ Admin has returned a receipt for correction. Delete it and upload a corrected version.
            </p>
          )}
        </div>
      )}
      ...
      {pendingFile && (
        <div className="ml-5 rounded-lg border border-emerald-200 bg-emerald-50 p-3 space-y-3">
          ...
          {/* OR Number & Date — stacked vertically so date input is full width */}
