...
const formatBioinformaticsDetailsHtml = (details: Record<string, any> | undefined): string => {
  ...
  const serviceTypes = Array.isArray(details.serviceTypes) ? details.serviceTypes.join(", ") : "";
  const fileFormats = Array.isArray(details.dataFileFormats) ? details.dataFileFormats.join(", ") : "";

  return `
    ${serviceTypes ? `
    <tr>
      <td style="padding: 4px 0; color: #64748b;">Type of bioinformatics service:</td>
      <td style="padding: 4px 0;">${serviceTypes}</td>
    </tr>` : ""}
    <tr>
      <td style="padding: 4px 0; color: #64748b;">Provide own data:</td>
      <td style="padding: 4px 0;">${details.dataProvideOwnData ? "Yes" : "No"}</td>
    </tr>
    ${details.dataProvideOwnData ? `
    <tr>
      <td style="padding: 4px 0; color: #64748b;">File format:</td>
      <td style="padding: 4px 0;">${fileFormats || "â€”"}${details.dataOtherFormat ? `; Others: ${details.dataOtherFormat}` : ""}</td>
    </tr>
    <tr>
      <td style="padding: 4px 0; color: #64748b;">Specify file size for each sample:</td>
      <td style="padding: 4px 0;">${details.dataFileSizePerSample || "â€”"}</td>
    </tr>
    <tr>
      <td style="padding: 4px 0; color: #64748b;">Preferred mode of file transfer:</td>
      <td style="padding: 4px 0;">${details.dataTransferMode || "â€”"}</td>
    </tr>` : ""}
    <tr>
      <td style="padding: 4px 0; color: #64748b;">Data generated by PGC sequencing service:</td>
      <td style="padding: 4px 0;">${details.dataProvidedByPgc ? "Yes" : "No"}</td>
    </tr>
  `;
};
...
const formatBioinformaticsDetailsText = (details: Record<string, any> | undefined): string => {
  ...
  return [
    serviceTypes ? `Type of bioinformatics service: ${serviceTypes}` : "",
    `Provide own data: ${details.dataProvideOwnData ? "Yes" : "No"}`,
    details.dataProvideOwnData
      ? `File format: ${fileFormats || "â€”"}${details.dataOtherFormat ? `; Others: ${details.dataOtherFormat}` : ""}`
      : "",
    details.dataProvideOwnData ? `Specify file size for each sample: ${details.dataFileSizePerSample || "â€”"}` : "",
    details.dataProvideOwnData ? `Preferred mode of file transfer: ${details.dataTransferMode || "â€”"}` : "",
    `Data generated by PGC sequencing service: ${details.dataProvidedByPgc ? "Yes" : "No"}`,
  ]
    .filter(Boolean)
    .join("\n");
};
