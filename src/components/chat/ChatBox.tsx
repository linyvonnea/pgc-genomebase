...
import { subscribeToThreadMessages, addThreadMessage, markMessagesAsRead } from "@/services/quotationThreadService";
...
export default function ChatBox({ inquiryId, role, variant = "default" }: ChatBoxProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const unsubscribe = subscribeToThreadMessages(inquiryId, (latestMessages) => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const senderDisplayName = role === "admin" 
    ...
    <Card className={`flex flex-col flex-1 relative ${variant === 'floating' ? 'h-full border-none shadow-none rounded-none' : 'h-[500px] shadow-sm border-gray-200'}`}>
        ...
        >
          {loading ? (
            ...
            <div className="flex flex-col justify-center items-center h-full text-center text-gray-500 space-y-2">
              ...
              <p className="text-xs text-gray-400">Send a message to start communicating.</p>
            </div>
          ) : (
                ...
                >
                  <div className={`flex flex-col max-w-[85%] ${isMe ? "items-end" : "items-start"}`}>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {msg.senderRole === (role === "admin" ? "client" : "admin") && (
                         <Badge variant="outline" className={`ml-1 text-[9px] h-4 py-0 px-1.5 ${
      ...
      <CardFooter className="p-3 bg-white border-t rounded-b-lg">
        <form onSubmit={handleSendMessage} className="flex w-full gap-2">
          <Input
            placeholder={role === "admin" ? "Message client..." : "Message admin..."}
            value={newMessage}
            disabled={loading}
